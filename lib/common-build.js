...
const commonBuild = ( options )=>{
    ...
    let {targetBuildType, backendProjectDir, distDirPath, frontendProjectDir, ftpOptions} = options;
    ...
    if(buildTypes.includes(validTypes.frontend) || buildTypes.includes(validTypes.backend)){
        // 清空dist目录
    ...
    if (buildTypes.includes(validTypes.backend)) {
        ...
        const content = fs.readFileSync(distConfigFile).toString()
        const prdContent = content.replace(`const env = 'devLocal'`, `const env = 'prd'`)
        fs.writeFileSync(distConfigFile, prdContent)
        console.log(chalk.blue('已将env修改为prd'))
    
        // 复制后端资源
        fs.copyFileSync(path.join(backendProjectDir, 'package.json'), path.join(distDirPath, 'package.json'))
        fs.copyFileSync(path.join(backendProjectDir, 'package-lock.json'), path.join(distDirPath, 'package-lock.json'))
        fs.copyFileSync(path.join(backendProjectDir, 'scripts/ecosystem.config.js'), path.join(distDirPath, 'ecosystem.config.js'))
        cpAssets({
            ignoreDirList: [
    ...
    if (buildTypes.includes(validTypes.frontend)) {
        ...
        try {
            execSync('npm run build', { cwd: frontendProjectDir, stdio: 'inherit' });
            console.log(chalk.blue('前端编译完成'))
        } catch (error) {
            console.log(chalk.red('编译前端工程发生错误') + frontendProjectDir);
            return;
        }
    ...
    if (buildTypes.includes(validTypes.ftp)) {
        ...
        ftp(distDirPath, serverDir, {...ftpOptions, cb: ()=>{
            const end = Date.now();
